...
backups, catalog, clone, close, create, documents, index, nobackups, open, process-xinclude, save, stream, switch-to-new-documents


=head1 TREE NAVIGATION

With XSH2, it is possible to browse a B<document tree> (XML data represented as a DOM-tree) as if it was a local f ilesystem, except that B<XPath> expressions are used instead of ordinary directory paths.
...
canonical, cd, fold, locate, ls, pwd, register-function, register-namespace, register-xhtml-namespace, register-xsh-namespace, unfold, unregister-function, unregister-namespace
...
clone, copy, edit, edit-string, insert, map, move, normalize, process-xinclude, remove, rename, set-dtd, set-enc, set-standalone, sort, strip-whitespace, wrap, wrap-span, xcopy, xinsert, xmove, xslt, xupdate
...
XSH2 variables are either globaly or lexically scoped. Global variables need not to be declared (they can be directly assigned to), whereas lexical variables must be declared using the command B<my>. Global variable assignment may also be made temporal for the enclosing block, using B<local>.
...
Localizing a variable using the B<local> keyword makes all assignments to it ocurring in the enclosing block temporary. The variable itself remains global, only its original value is restored at the end of the block that localized it.
...
The commands listed below can be used to modify the default behaviour of the XML parser or XSH2 itself. Some of the commands switch between two different modes according to a given expression (which is expected to result either in zero or non-zero value). Other commands also working as a flip-flop have their own explicit counterparts (e.g. B<verbose> and B<quiet> or B<debug> and B<nodebug>). This misconsistency is due to historical reasons.
...
The B<settings> command displays current settings by means of XSH2 commands. Thus it can not only be used to review current values, but also to store them for future use, e.g. in ~E<sol>.xshrc file.
...
To prevent conflict between XSH2 internals and the evaluated Oerl code, XSH2 runs such code in the context of a special namespace B<XML::XSH2::Map>. As described in the section B<Variables>, XSH2 string variables may be accessed and possibly assigned from Perl code in the most obvious way, since they actually are Perl variables defined in the B<XML::XSH2::Map> namespace.
...
Print all help topics containing given expression in their short description. The B<--fulltext> flag forces the search to be performed over the full text of help. B<--fulltext> indicates, that the given B<expression> is a regular expression instead of a literal string.
...
[assign] B<$variable> = B<expression>
             ...
             [assign] B<$variable> [-= E<verbar> += E<verbar> *= E<verbar> E<sol>= E<verbar> %= E<verbar> x= E<verbar> .= E<verbar> E<verbar>E<verbar>= E<verbar>
 &&= ] B<expression>
...
Call a subroutine whose name is computed by evauluating the first argument B<expression>. All other expressions are evaluated too and the results are passed to the subroutine as arguments.
...
Note that XSH2 refuses to create multiple top-level elements using B<copy>, B<move> and smilar commands.

Example: Replace living-thing elements in the document b with copies of the coresponding creature elements from the document $a.
...
def B<subroutine> [B<$variable> ...] B<command-or-block>
...
Define a new XSH2 sub-routine named B<subroutine>. The subroutine may require zero or more parameters. These are declared as a whitespace-separated list of parametric variables. The body of the subroutine is specified as a B<command-or-block>.
...
do B<command-or-block>
             


=item Description:

Execute B<command-or-block>. This command is probably only useful when one wants to redirect output of more than one command.
...
If the result saved by the editor is empty, the interactive XSH2 shell asks user to confirm this was correct. This confirmation can be supressed using B<--allow-empty> or B<:0> (zero) options.
...
NOTE: This command has very different behavior from XSH1 eval alias for B<perl>.
...
This feature is still EXPERIMENTAL and may change in the future! Fold command may be used to mark elements with a B<xsh:fold> attribute from the B<http:E<sol>E<sol>xsh.sourceforge.netE<sol>xshE<sol>> namespace. When listing the DOM tree using B<B<ls> --fold B<xpath>>, elements marked in this way are folded to a given depth (default is 0 = fold immediately).
...
foreach B<expression> 
    B<command>E<verbar>B<command-or-block>
...
Evaluate given B<expression> to a node-list and for each resulting node execute given B<command> or B<command-or-block>. If used without a loop B<$variable>, the loop temporarily sets current node to the node being processed. Otherwise, the processed node is assigned to the loop variable.
...
if B<expression> B<command>
             if B<expression>
    B<command-or-block> [ elsif B<command-or-block> ]* [ else B<command-or-block> ]
             


=item Description:

Executes B<command-or-block> if a given B<expression> expression evaluates to a non-emtpty node-list, true boolean-value, non-zero number or non-empty literal. If the first expression fails, then B<elsif> conditions are tested (if any) and the B<command-or-block> corresponding to the first one of them which is true is executed. If none of the conditions is satisfied, an optional B<else> B<command-or-block> is executed.
...
Note, that since the underlying C library (libxml2) uses a hardcoded indentation of 2 space characters per indentation level, the amount of whitespace used for indentation can not be altered at runtime.
...
The following two examples give equivallent results. However, the one using B<iterate> may be faster if the number of nodes being counted is huge and document order isn't indexed.
...
This command acts in a very similar way as B<assign> does, except that the variable assignment is done temporarily and lasts only for the rest of its enclosing B<command-or-block> or subroutine. At the end of the enclosing block or subroutine, the original value is restored. This also reverts any later usual assignments to the variable done occurring before the end of the block. This command may also be used without the assignment part.

Note, that the variable itself remains global in the sense that it is still visible to any subroutine called subsequently from the same block. Unlike B<my> declaration, it does not create a new lexicaly scoped variable.
...
ls [--foldE<verbar>:f] [--fold-attrsE<verbar>:A] [--indentE<verbar>:i E<verbar> --no-indentE<verbar>:n]
...
B<--indent> (B<:i>) and B<--no-indent> (B<:n>) may be used to enforceE<sol>suppress indentation, overriding current setting (see command B<indent>).
...
The B<expression> in the second argument is evaluated to a node-list whose each node passes its data to the B<perl-code> via the B<$_> variable and loads back the (possibly modified) data from the same variable.

Since element nodes do not actually bear any data (they are just containers for other nodes), their names (tag) is used instead. Note, however, that there is a dedicated command B<rename> with a very similar syntax to B<map>, that should be used for renaming nodes like elements, attributes, and processing instructions.
...
This command returns a node-list consisiting of nodes it created on the target locations.
...
namespaces [B<expression>]
             


=item Description:

For each node in a given node-list lists all namespaces that are valid the scope of the node. Namespaces are listed in the form of B<xmlns:prefix="uri"> declarations. If no B<xpath> is given, lists namespaces in the scope of the current node.
...
print [--nonlE<verbar>:n] [--stderrE<verbar>:e] B<expression> [B<expression> ...]
...
rename B<perl-code> B<expression>
             


=item Description:

This command is very similar to the B<map> command, except that it operates on nodes' names rather than their dataE<sol>values. For every element, attribute or processing-instruction contained in the node-list specified by a given B<expression>, the following procedure is used: 1) the name of the node is stored into Perl's B<$_> variable, 2) the B<perl-code> is evaluated, and 3) the (posibly modified) content of the B<$_> variable is used as a new name for the node.
...
This command immediatelly stops the execution of a procedure it occurs in and returns the execution to the place of the script from which the subroutine was called. Optional argument may be used as a return value for the subroutine call.
...
B<--format> option may be used to specify the output format. It's argument should be either B<xml>, B<html> or an expression evaluating to one of these. If not specified, XML output is assumed. Note, that a document should be saved as HTML only if it actually is a HTML document, the result would be an unparsable instance. Note also, that the optional encoding parameter only forces character conversion; it is up to the user to declare the document encoding in the appropriate HTML E<lt>METAE<gt> tag, if needed.
...
B<--indent> (B<:i>) and B<--no-indent> (B<:n>) may be used to enforceE<sol>suppress indentation, overriding current global setting of B<indent>.
...
B<--numericE<verbar>:n> specifies, that keys should be compared by their numerial values (the default is string comparison).
...
stream [ --input-fileE<verbar>:f B<filename> E<verbar>
   ...
   [ --output-fileE<verbar>:F B<filename> E<verbar>
           ...
           select B<xpath> B<command-or-block>
         [ select B<xpath> B<command-or-block> ... ]
...
EXPERIMENTAL! This command provides a memory efficient (though slower) way to process selected parts of an XML document with XSH2. A streaming XML parser (SAX parser) is used to parse the input. The parser has two states which will be refered to as A and B below. The initial state of the parser is A.
...
In state B the parser builds a complete DOM subtree of the element that was last added to the tree before the parser changed its state from A to B. No data are sent to the output at this stage. When the subtree is complete (i.e. the corresponding end-tag for its topmost element is encountered), the B<command-or-block> of instructions following the B<xpath> expression that matched is invoked with the root element of the subtree as the current context node. The commands in B<command-or-block> are allowed to transform the whole element subtree or even to replace it with a different DOM subtree or subtrees. They must, however, leave intact all ancestor nodes of the subtree. Failing to do so can result in an error or unpredictable results.

After the subtree processing B<command-or-block> returns, all subtrees that now appear in the DOM tree in the place of the original subtree are serialized to the output stream. After that, they are deleted and the parser returns to state A.
...
try B<command-or-block> catch [[localE<verbar>my] B<$variable>] B<command-or-block>
             


=item Description:

Execute the B<command-or-block> following the B<try> keyword. If an error or exception occures during the evaluation, execute the B<catch> B<command-or-block>. If the B<catch> keyword is followed by a variable (possibly localized for the following block using B<my> or B<local>) and the B<try> block fails with an exception, the error message of the exception is stored to the variable before the B<catch> block is executed.
...
unless B<expression>
    B<command>
             unless B<expression>
    B<command-or-block> [ else B<command-or-block> ]
...
B<--schema> or B<:S> forces W3C XML Schema validation (XSD). Support for schema validation may still be inclomplete (see libxml2 homepage for more details).
...
while B<expression> B<command-or-block>
             


=item Description:

Execute the B<command-or-block> as long as the given B<expression> evaluates to a non-emtpty node-list, true boolean-value, non-zero number or non-empty literal.
...
wrap [--namespace B<expression>] [--inner] B<expression> B<xpath>
             


=item Description:

For each node matching B<xpath>, this command creates a new element node according to a given B<expression> (in the same way as B<xinsert> does) to replace the matching node and moves the matching node into the newly created element. If namespace B<expression> is given, the namespace is applied on the created element. The command returns a node-list consisting of the elements created.

With B<--inner> (or B<:i>) flag the command wraps children nodes of the matching node rather than the node it self the following sense: for each matching node a new element node is created, but this time it is placed into the matching node and all previous children of the matching node are moved into the newly created node. In this mode, all non-element matching nodes are ignored.
...
xcopy [--respectiveE<verbar>:r] B<expression> B<location> B<expression>
...
xmove [--respectiveE<verbar>:r] B<xpath> B<location> B<xpath>
...
This command returns a node-list consisiting of all nodes it created on the target locations.
...
If B<--compile> or B<:c> is used, compile a given XSLT stylesshet and return a compiled XSLT stylesheet object. This object can be later passed as a XSLT stylesheet to B<xslt --precompiled>.
...
All arguments following the second (document) argument are considered to be stylesheet parameters and (after expanding B<${...}> interpolators) are directly passed to the XSLT engine without being evaluated by XSH2. All stylesheet parameters should be of the form B<name=B<xpath>> (possibly in brackets).
...
XUpdate langauge is described in XUpdate Working Draft at http:E<sol>E<sol>xmldb-org.sourceforge.netE<sol>xupdateE<sol>xupdate-wd.html.
...
Interpolators of the form B<${{> and B<}}> evaluate their contents as a Perl expresson (in very much the same way as the B<perl> command) and interpolate to the resulting value.
...
An B<expression> which evaulates to a valid filename or URL. As long as the expression contains no whitespace, no brackets of any type, quotes, double-quotes, B<$> character nor B<@> character, it is treated as a literal token which evaluates to itself.
...
An B<expression> which evaulates to a valid name of an element, attribute or processing-instruction node. As long as the expression contains no whitespace, no brackets of any type, quotes, double-quotes, B<$> character, nor B<@> character, it is treated as a literal token which evaluates to itself.
...
avoiding any white-space outside the squere brackets, or completely enclosed in brackets as in

XSH2 provides a number of powerfull XPath extension functions, listed below and described in separate sections, see xsh:current, xsh:doc, xsh:grep, xsh:id2, xsh:if, xsh:join, xsh:lc, xsh:lcfirst, xsh:map, xsh:matches, xsh:max, xsh:min, xsh:new-attribute, xsh:new-cdata, xsh:new-chunk, xsh:new-comment, xsh:new-element, xsh:new-element-ns, xsh:new-pi, xsh:new-text, xsh:parse, xsh:path, xsh:reverse, xsh:same, xsh:serialize, xsh:split, xsh:sprintf, xsh:strmax, xsh:strmin, xsh:subst, xsh:substr, xsh:sum, xsh:times, xsh:uc, xsh:ucfirst, xsh:var. XPath extension functions by default belong to XSH2 namespace B<http:E<sol>E<sol>xsh.sourceforge.netE<sol>xshE<sol>> with a namespace prefix set to B<xsh>. A program may however call the B<xpath-extensions> command to map XSH2 XPath extension functions into the default namespace, so that they may be used directly without any prefix.
...
This function returns a string containing cannonical XPath leading to B<NODE>.
...
This function provides direct access to the very powerfull Perl function B<split>. It splits B<STRING> to a list of fields. B<PATTERN> is a regular expression specifing strings delimiting individual fields of B<STRING>. If B<PATTERN> is empty, B<STRING> is split to individual characters. If the regular expression in B<PATTERN> is enclosed in brackets, then strings matching B<PATTERN> are also included in the resulting list.
